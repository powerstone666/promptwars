  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      <section className="relative bg-[#0a0a10] py-24 px-6 md:px-12 z-30">
        <div className="max-w-4xl mx-auto bg-[var(--mr-surface-low)] border border-white/5 p-8 md:p-12 rounded-xl shadow-2xl overflow-hidden">
          <ReactMarkdown
            components={{
              h1: ({ node, ...props }) => <h1 className="text-4xl md:text-5xl font-black italic mt-8 mb-6 text-white tracking-widest" style={{ fontFamily: "var(--font-headline)" }} {...props} />,
              h2: ({ node, ...props }) => <h2 className="text-3xl md:text-4xl font-black italic mt-12 mb-4 text-[var(--mr-gold)] border-b border-white/10 pb-2" style={{ fontFamily: "var(--font-headline)" }} {...props} />,
              h3: ({ node, ...props }) => <h3 className="text-2xl font-bold mt-8 mb-3 text-white italic" style={{ fontFamily: "var(--font-headline)" }} {...props} />,
              h4: ({ node, ...props }) => <h4 className="text-xl font-bold mt-6 mb-2 text-white" {...props} />,
              p: ({ node, ...props }) => <p className="mb-6 text-[var(--mr-text-muted)] leading-relaxed text-lg" {...props} />,
              ul: ({ node, ...props }) => <ul className="list-disc pl-8 mb-6 text-[var(--mr-text-muted)] space-y-2 text-lg" {...props} />,
              ol: ({ node, ...props }) => <ol className="list-decimal pl-8 mb-6 text-[var(--mr-text-muted)] space-y-2 text-lg" {...props} />,
              li: ({ node, ...props }) => <li className="" {...props} />,
              code: ({ node, inline, ...props }: any) =>
                inline ? (
                  <code className="bg-black/40 px-1.5 py-0.5 rounded text-sm font-mono text-[var(--mr-cyan)] border border-white/10" {...props} />
                ) : (
                  <div className="relative mb-6">
                    <pre className="bg-black/60 p-6 rounded-lg overflow-x-auto border border-white/10 shadow-inner">
                      <code className="font-mono text-sm text-[var(--mr-text-dim)]" {...props} />
                    </pre>
                  </div>
                ),
              a: ({ node, ...props }) => <a className="text-[var(--mr-gold)] hover:underline font-medium" {...props} />,
              strong: ({ node, ...props }) => <strong className="font-bold text-white tracking-wide" {...props} />,
              blockquote: ({ node, ...props }) => <blockquote className="border-l-4 border-[var(--mr-gold)] pl-6 italic text-[var(--mr-text-dim)] bg-white/5 p-4 rounded-r-lg mb-6" {...props} />,
              ...
              th: ({ node, ...props }) => <th className="border-b border-white/10 py-3 px-4 text-white font-bold" {...props} />,
              td: ({ node, ...props }) => <td className="border-b border-white/5 py-3 px-4 text-[var(--mr-text-muted)]" {...props} />,
            }}
          >
            {agentsMdContent}
          </ReactMarkdown>
